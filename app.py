...
def venues():
  ...
  for city in cities:
      venue = db.session.query(Venue.id, Venue.name).filter(Venue.city == city[0]).filter(Venue.state == city[1])
      data.append({
        "city": city[0],
        "state": city[1],
        "venues": venue
...
# Done: replace with real artist data from the artist table, using artist
